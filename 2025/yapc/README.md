...
^ 心理的依存が増えると、これらの次元のスコアが上がり、バーンアウトへと進行する

---

#　依存をなくすのは難しい

- {技術的, 心理的}依存は「どちらか一方」ではなく、バランスの問題
- 心理的依存を技術的依存に変換できる部分は存在する
- 📝 技術的に解決できない問題は存在する
...
^ 心理的依存はコントロールが難しく特定の人に負荷が高まる
^ 技術的依存はコントロールしやすく特定の人に負荷が分散される
^ 継続していくには、技術的依存を増やし心理的依存を減らすことが重要
^ 一方で、技術的依存にはメンテナンスコストが伴うため、バランスが必要

---

# [fit] 技術的依存を増やし、心理的依存を減らす

- 心理的依存は自覚も難しく、コントロールも難しい
- 技術的依存は設計次第で、何に依存しているのかが明確
- 全てを技術的依存に変えることは不可能だが、
- → 技術的依存を増やし、心理的依存を減らす設計が重要
...
![inline fit](img/jser-automation-flow.png)
...
![inline fit](img/jser-automation-flow-now.png)
...
- コアにルールを持つとルールのIssueもコアに集まる = 心理的負担が増える
...
- 例えば「冗長な表現です」というルールがあった場合に、なぜ冗長なのか？どう直せば良いのか？を説明するのが難しい
- すなわち、心理的負担が大きい部分がある
...
![fit promptfooでのLLM as Judgeの例](img/promptfoo-textlint.png)
...
- jsprimerでOpen Collectiveを開始したのは2023年から
- 書き始めてから8年ぐらい経ってから
- Why? -> Open Collectiveを初めて心理的負荷が増えないと思えるようになるぐらい安定した
- 目的は、あくまで変化に対応できること、お金を得ることは目的ではない
- Contributeをしやすくするという観点を見たときに、そこへプールする仕組みがあった方がもっと自由度は高くなる
- お金はフローではなく、ストックで考える。オープンソースでも貯めることはできるし、毎年使い果たす必要は別にない
- それをどうやったら上手く活用できるのかという視点で考えたときに、もっと面白いことがあると思って始めた

---

- GitHub Sponsorsも同様
- Open Collectiveはプロジェクト向け
- GitHub Sponsorsの方は個人に近い(個人間の手数料がないため、企業からのスポンサーはOpen Collectiveの方がやりやすいが)
...
# Disclamr: 持続することがなぜ大事? この方法がいいというわけではないが
