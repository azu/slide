...
- (イベントソーシング)は抜いてCQRSについて考えAlminを実装した
- ドメイン/ビジネスロジックをしっかり考えて実装できるようにした
...
- イベント駆動が主流のJavaScriptにおいて、CQRS+ESは一つの到達点かもしれないが次は考えないといけない
- 次を考えられるような状態をつくるために色々設計して考えないと行けない
- クライアントサイド|フロントエンドとサーバサイド|バックエンドでは、必ずしもベストなアーキテクチャが一致する訳ではない

-----

- JavaScriptにおいてCQRS + ESを管理する優れたProcess Managerがまだ不足している
- それがRedux-SagaのようなGeneratorなのか、RxJSのようなStreamなのかはまだわからない
